...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="xml/fs">
        ...
        <!-- 2024-07-23 Column positioning not deployed yet...-->
        <g id="{@feats}">
         ...
         <xsl:comment>LinkInfo VALUE: <xsl:value-of select="$linkInfo"/></xsl:comment> 
           ...
           <xsl:choose> 
              ...
              <xsl:otherwise> 
                  ...
                  <a xlink:href="{$linkConstructor}"><line x1="{position() * 150}" x2="{position() * 150}" y1="{$yPos}" y2="{$yPos + 30}" stroke-width="100" stroke="rgb({$heatMapVal}, {255 - 2*$heatMapVal}, {255 - 2*$heatMapVal})">
